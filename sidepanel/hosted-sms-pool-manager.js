(function attachSidepanelHostedSmsPoolManager(globalScope) {
  ...
  function createHostedSmsPoolManager(context = {}) {
      ...
      return getEntriesWithState(entries).filter((entry) => {
        const matchesFilter = (() => {
          switch (filterMode) {
            ...
            case 'cooldown': return Boolean(entry.disabled);
    ...
    function updateControls(entries = renderedEntries) {
      ...
      const cooldownCount = entriesWithState.filter((entry) => entry.disabled).length;
      const unavailableCount = entriesWithState.filter((entry) => entry.unavailableCount > 0).length;
      if (dom.btnHostedSmsPoolClearUsed) {
        dom.btnHostedSmsPoolClearUsed.disabled = loading
          || (usedCount === 0 && disabledCount === 0 && cooldownCount === 0 && unavailableCount === 0);
      }
    ...
    function render(entries = parseEntries(state.getText?.())) {
      ...
      dom.hostedSmsPoolSummary.textContent = `已加载 ${entriesWithState.length} 个号码，${usedCount} 个有使用记录，${disabledCount} 个已禁用，${cooldownCount} 个冷却中，${unavailableCount} 个不可用，累计使用 ${totalUseCount} 次。`;
      ...
      for (const entry of visibleEntries) {
        ...
        item.className = `luckmail-item${entry.current ? ' is-current' : ''}${(!entry.enabled || entry.disabled) ? ' is-disabled' : ''}`;
